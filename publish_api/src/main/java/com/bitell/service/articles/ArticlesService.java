...
import com.bitell.mapper.CArticlesForwardsMapper;
...
public class ArticlesService implements ArticlesServiceI {
    ...
    @Autowired
    CArticlesForwardsMapper cArticlesForwardsMapper;
    @Autowired
...
//    }
    ...
    public ArticleRelatedPublishResponse articleRelatedPublish(ArticleRelatedPublishRequest articleRelatedPublishRequest) throws Exception {
        try{
            if(CollectionUtils.isEmpty(articleRelatedPublishRequest.getSubjectIds())){
                return new ArticleRelatedPublishResponse(ApiError.SUBJECTIDS_IS_NULL,ApiError.SUBJECTIDS_IS_NULL_MESSAGE);
            }
            //插入文章表
            CArticles insertCArticles = new CArticles();
            BeanUtils.copyProperties(articleRelatedPublishRequest,insertCArticles);
            cArticlesMapper.insertSelective(insertCArticles);
            //添加主题与文章的关系
            articleRelatedPublishRequest.getSubjectIds().stream().forEach(sid->{
                ArticleSubRef insertArticleSubRef = new ArticleSubRef();
                insertArticleSubRef.setArticleId(insertCArticles.getId());
                insertArticleSubRef.setSubId(sid);
                articleSubRefMapper.insertSelective(insertArticleSubRef);
            ...
            //插入转发内容
            CArticlesForwardsVo cArticlesForwardsVo  =articleRelatedPublishRequest.getcArticlesForwards();
            if (null != cArticlesForwardsVo){
                CArticlesForwards insertCArticlesForwards = new CArticlesForwards();
                BeanUtils.copyProperties(cArticlesForwardsVo,insertCArticlesForwards);
                insertCArticlesForwards.setArticlesId(insertCArticles.getId());
                cArticlesForwardsMapper.insertSelective(insertCArticlesForwards);
            }
            return new ArticleRelatedPublishResponse(ApiError.OK,ApiError.OK_MESSAGE);
        }catch (Exception e){
            e.printStackTrace();
